...
function Bot() {
    ...
    const handleSendMessage = async () => {
        ...
        try {
           const res=await axios.post("https://chatfusion-3mf6.onrender.com/bot/v1/message",{
                text: input
            })
            if(res.status === 200) {
    ...
    <div className='flex flex-col min-h-screen bg-[#0d0d0d] text-white'>
      ...
      <header className="fixed top-0 left-0 w-full border-b border-gray-800 bg-[#0d0d0d] z-10">
        <div className=" container mx-auto flex justify-between items-center px-6 py-4">
          <h1 className="text-lg font-bold">ChatFusion</h1>
        ...
        <div className="w-full max-w-4xl mx-auto px-4 flex flex-col space-y-3">
          {messages.length === 0 ? (
            ...
            <div className="text-center text-gray-400 text-lg">
              👋 Hi, I'm{" "}
              <span className="text-green-500 font-semibold">ChatFusion</span>.
            </div>
          ) : (
            <>
